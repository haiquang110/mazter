 ...
 *
 *    $sftp = new \mazter\Net\SFTP('www.domain.tld');
...
class SFTP extends SSH2
{
     ...
     *
     * \mazter\Net\SSH2::exec() uses 0 and \mazter\Net\SSH2::read() / \mazter\Net\SSH2::write() use 1.
     *
    ...
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        // the order, in this case, matters quite a lot - see \mazter\Net\SFTP::_parseAttributes() to understand why
        ...
        // see \mazter\Net\SFTP::_parseLongname() for an explanation
    ...
    function _init_sftp_connection()
    {
        ...
        /*
         ...
         in draft-ietf-secsh-filexfer-13 would be quite impossible.  As such, what \mazter\Net\SFTP would do is close the
    ...
    function chdir($dir)
    {
        ...
        // see \mazter\Net\SFTP::nlist() for a more thorough explanation of the following
        $response = $this->_get_sftp_packet();
        switch ($this->packet_type) {
    ...
    function _update_stat_cache($path, $value)
    {
        ...
        foreach ($dirs as $i => $dir) {
            ...
            //  1. a file was deleted and changed to a directory behind mazter's back
     ...
     * Returns general information about a file or symbolic link
     *
     * Determines information without calling \mazter\Net\SFTP::realpath().
    ...
    function chown($filename, $uid, $recursive = false)
    {
        /*
         ...
         mazter _could_ auto append the dns_domain to $uid BUT what if it shouldn't
         have one? mazter would have no way of knowing so rather than guess mazter
     ...
     *
     * By default, \mazter\Net\SFTP::put() does not read from the local filesystem.  $data is dumped directly into $remote_file.
     * So, for example, if you set $data to 'filename.ext' and then do \mazter\Net\SFTP::get(), you will get a file, twelve bytes
     ...
     * @return bool
     * @access public
     * @internal ASCII mode for SFTPv4/5/6 can be supported by adding a new function - \mazter\Net\SFTP::setMode().
     */
